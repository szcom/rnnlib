...
parser.add_option("-m", "--maxarraysize", action="store", type="int", dest="maxArraySize", default=8000000,
                  help="maximum array size for std and mean calcs")
parser.add_option("-i", "--inputarrayname", action="store", type="string", dest="inputArrayName", default="inputs",
                  help="name of input array")
parser.add_option("-o", "--outputarrayname", action="store", type="string", dest="outputArrayName", default="inputs",
                  help="name of output array")
parser.add_option("-s", "--stdmeanfilename", action="store", type="string", dest="stdMeanFilename", default="",
                  help="file to use for stds and means")
parser.add_option("-b", "--bigfile", action="store_true", dest="bigFile", default=False,
                  help="use memory optimisations for big files? (slower)")
parser.add_option("-c", "--booleanColomn", action="store", type="int", dest="booleanColomn", default=-1,
                  help="dont normalize nth colomn of input array")


def std(array, axis):
    if np.shape(array)[axis] > 1:
...
infile = netcdf_helpers.netcdf_file(inputFilename, 'r')

print "loading in input array"
inputVar = infile.variables[options.inputArrayName]
outputArray = np.zeros(inputVar.shape, 'f')
if options.bigFile:
    offset = 0
    ...
    outputArray = np.copy(inputVar[:])
    outputArray.flags.writeable = True

if options.stdMeanFilename != "":
    print "reading std deviations and means from", options.stdMeanFilename
    stdMeanFile = netcdf_helpers.netcdf_file(options.stdMeanFilename, 'r')
    inputStds = np.array(stdMeanFile.variables[options.inputArrayName + 'Stds'][:])
    inputMeans = np.array(stdMeanFile.variables[options.inputArrayName + 'Means'][:])
else:
    print "calculating std deviations"
    inputStds = std(outputArray[:options.maxArraySize], 0)
    print "calculating means"
    inputMeans = np.mean(outputArray[:options.maxArraySize], 0)

if options.booleanColomn > 0:
    print "dont normalize boolean column", options.booleanColomn
